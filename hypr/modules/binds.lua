if type(rebinds) ~= "table" then rebinds = {} end
...
-- overview). On desktop 2, Super+3 -> ws13, never ws3, so each desktop keeps
-- its own 1..10 and windows never jump desktops. The helper also pulls the target
-- to the monitor under the cursor, so the keys drive whichever screen the mouse
-- is on. Super+Alt+N sends the active window to that slot, staying on this desktop.
...
hl.bind(K(mod .. " + J"),          hl.dsp.workspace.toggle_special("music"))       -- toggle the music scratchpad
